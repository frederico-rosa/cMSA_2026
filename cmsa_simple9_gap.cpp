// =============================================================
// cmsa_simple9_gap.cpp
//
// cMSA comprimida com Simple-9 (words de 32 bits), codificando o
// gap (diferenca) entre objetos.
//
...
// utilizada primeiro para contabilização do tamanho e posteriormente para construir o código Simple-9
static void constructSimple9(const std::vector<int>& pos, long long obj, const Context& ctx,
                              std::vector<bool>& firstElement, std::vector<int>& lastValue,
                              std::vector<Simple9Encoder>& buckets) {

    for (int k = 0; k < ctx.n; k++) {

        // obj pode passar de 25 milhoes e ctx.n=128: obj*ctx.n sozinho ja
        // excede o limite de um int de 32 bits (2^31-1). Fazemos a conta
        // em 64 bits e só then truncamos para 32 bits (comportamento
        // definido em static_cast, ao contrário do overflow de int*int,
        // que é UB). Como só o "diff" entre valores consecutivos é
        // gravado (e ele é sempre pequeno), a truncação não afeta o
        // resultado — mas fazer a multiplicação em 32 bits era UB.
        long long value64 = obj * static_cast<long long>(ctx.n) + pos[k];
...
static void searchByObject(const std::vector<int>& lq, std::vector<Simple9Decoder>& dec,
    ...
    for (int i = 0; i < ctx.nn; i++) {
        ...
        for (int k = 0; k < ctx.n; k++) {
            ...
            int refPos = 0;
            if (firstElement[k]) {
                // Simple9 não precisa do "-1": diferente do código Delta,
                // ele consegue representar 0 diretamente, então o valor
                // gravado no construct já é o valor "puro" (sem +1).
                int msaValue = dec[bucketID].next() % ctx.n;
                lastValue[k] = msaValue;
                firstElement[k] = false;
                refPos = msaValue;

            } else {
                int decodedValue = dec[bucketID].next();
                int msaValue = decodedValue + lastValue[k];
                // msaValue reconstroi obj*ctx.n+pos[k] truncado em 32 bits;
                // uma vez que obj*ctx.n+pos[k] ultrapassa 2^31-1, o valor
                // truncado vira negativo (reinterpretacao em complemento de
                // dois) e o operador % do C++ (que trunca em direcao a
                // zero) devolve resto negativo em vez do modulo euclidiano
                // esperado. Normaliza para o intervalo [0, ctx.n).
                int value = msaValue % ctx.n;
                if (value < 0) {
                    value += ctx.n;
                }
                lastValue[k] = msaValue;
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "cmsa_simple9_gap.cpp" << std::endl;
	std::cout << "Programa: cMSA simple9 direto em RAM (sem armazenar em disco)" << std::endl;
    ...
    // primeiros elementos são codificados, proximos elementos, as diferenças
    std::vector<bool> firstElement(ctx.n, true);

    // usado para operar as diferenças dos elementos vizinhos em bucket
    std::vector<int> lastValue(ctx.n, 0);
    ...
    for (long long i = 0; i < totalDimNn; i++) {
        ...
        if (i % ctx.dim == ctx.dim - 1) {
            ...
            constructSimple9(pos, i / ctx.dim, ctx, firstElement, lastValue, counters);
        }
    }
    ...
    std::fill(firstElement.begin(), firstElement.end(), true);
    ...
    for (long long i = 0; i < totalDimNn; i++) {
        ...
        if (i % ctx.dim == ctx.dim - 1) {
            std::vector<int> loi = getOrderedList(ctx, oi, r);
            std::vector<int> pos = invertPositions(ctx, loi);
            constructSimple9(pos, i / ctx.dim, ctx, firstElement, lastValue, bucketsS9);
        }
    }
    ...
    for (int qid = 0; qid < ctx.num_q; qid++) {
        ...
        searchByObject(lq, dec, firstElement, lastValue, ctx);
